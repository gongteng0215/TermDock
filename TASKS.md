...
- `P0-F4` 已接入 electron-builder + GitHub Release 工作流并支持 mac `arm64/x64`，待签名/公证策略完善
...
1. `P0-F3`：执行一轮完整跨平台冒烟（mac Intel/mac Apple Silicon/Windows）并沉淀 checklist
2. `P0-F4`：完善签名与公证策略（Windows 代码签名、mac Notarization）并验证安装体验
3. `P0-E3`：建立全局错误恢复指引（可重试按钮、断线提示、桥接异常恢复）
4. `P0-D4` + `P0-D6`：传输队列/并发策略 + 目录拖拽上传
5. `P0-A3`：SQLite 迁移（sessions/groups/recent_sessions）
