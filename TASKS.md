...
- `Compact-first`：后续新增页面、面板、弹窗、列表默认采用紧凑布局
- 高密度优先：单屏展示更多核心信息，减少滚动和切换
- 一致性要求：同类组件沿用统一的紧凑间距/字号规范，避免模块间风格漂移
...
- `DONE` `P0-C4` 多标签会话管理
- `DONE` `P0-C6` macOS 快捷键优先适配（Cmd+T/W/C/V/F）
- `DONE` `P0-C7` 同一会话多开（同 session 多 tab）
- `DONE` `P0-C8` 终端右键上下文菜单（含 Clear，结构可扩展）
- `DONE` `P0-D1` SFTP 通道封装（复用 SSH 连接）

### 部分完成（可用但未达验收）

- `PARTIAL` `P0-A3` 已实现本地持久化 CRUD（JSON），尚未迁移 SQLite / recent_sessions / groups
- `PARTIAL` `P0-B1` 已有会话列表，尚缺分组树与搜索过滤
- `PARTIAL` `P0-B2` 已支持创建与编辑表单，尚缺更完整校验与批量编辑体验
- `PARTIAL` `P0-B4` 已记录成功连接时间（`lastConnectedAt`），尚缺独立 `recent_sessions` 表与分组能力
- `PARTIAL` `P0-B5` 已支持收藏标记与“仅收藏”快速筛选，尚缺分组视图下的收藏体验打磨
- `PARTIAL` `P0-C5` 已配置 KeepAlive，尚缺断线重连交互
- `PARTIAL` `P0-D2` 右侧 SFTP 面板已接入目录浏览，尚缺折叠/布局交互完善
- `PARTIAL` `P0-D3` 已支持目录列表、进入目录与返回上级，尚缺完整文件信息与异常态体验
- `PARTIAL` `P0-D4` 已支持单文件上传/下载与进度显示，尚缺传输队列调度与并发策略
- `PARTIAL` `P0-D5` 已支持新建目录/重命名/删除（非递归），尚缺覆盖确认与批量流程
- `PARTIAL` `P0-D6` 已支持拖拽文件上传到当前目录，尚缺目录拖拽与批量交互打磨

### 未开始（下一步优先）

- `TODO` `P0-D6` 目录拖拽上传与高级传输体验
...
| P0-D3 | 文件列表与目录切换 | 1d | P0-D1, P0-D2 | 可进入目录并返回上级，列表采用类 `ls -l` 的紧凑信息展示 |
...
6. 每个里程碑做一次紧凑度回归检查，避免后续功能把信息密度做“松”。
