...
**给经常登录服务器的开发者用的本地优先 SSH + SFTP 桌面工作台。**

TermDock 把 SSH 终端、SFTP 文件浏览、失败传输恢复、服务器健康、端口转发和危险命令确认放在同一个桌面窗口里。不需要云账号；会话和凭据默认留在本机。
...
TermDock 面向的是每天反复连接服务器、查看状态、移动文件、处理失败任务的工作流。它不是云控制台，也不是“大而全”的终端套件，而是一个偏实用的本地服务器工作台。

### 你能得到什么

- **SSH + SFTP 同屏工作**：终端标签、远程文件、命令历史和传输队列放在一个工作区里。
- **失败传输恢复**：上传/下载失败后，可以直接在 Retry Center 里重试，不用手动重建队列。
- **工作时顺手看服务器健康**：登录后就能查看 CPU、内存、磁盘、网络、进程和失败服务。
- **危险命令保护**：高风险命令真正写入生产类主机前先停下来确认。

TermDock 当前主要服务个人开发者和小团队，在 Windows 和 macOS 上提供本地优先的 SSH/SFTP 工作流。
