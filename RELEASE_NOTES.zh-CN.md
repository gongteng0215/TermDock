...
- 新增工业风 Cockpit 主题，作为更克制、更紧凑的控制台方案，同时完整保留 Default 与原有科技风主题。
- 调整 Cockpit 窗口控制、面板层级、导航和紧凑列表节奏，在桌面及窄窗口尺寸下提升日常运维可读性。
- SFTP 支持 Ctrl/Cmd 与 Shift 多选，可批量下载、删除和复制路径；危险操作使用项目风格确认框。
- SFTP 长文件名与长路径采用安全省略、完整悬停提示和右键路径预览，避免挤压操作区。
- 新增按会话记忆的 SFTP 搜索、文件/文件夹筛选、名称/大小/修改时间排序，以及一键选择当前结果。
- 大型 SFTP 目录使用稳定虚拟列表，同时保持原有紧凑行高；定向验证覆盖 240 项目录。

### 验证

- `pnpm run typecheck` 已通过。
- `pnpm run build` 已通过。
- `pnpm run smoke:ui` 已通过：52 项通过，0 项失败。
- `pnpm run smoke:ui:packaged` 已通过：52 项通过，0 项失败。
- Industrial/SFTP 定向验证已通过：覆盖 240 个远程条目，1200px 下无横向溢出。
- Windows 打包与 GitHub Actions 发行产物由正式发布工作流验证。
...
- 界面密度设置：
  - 设置 > 工作区可选择「紧凑 / 宽松」。
  - 紧凑（默认）会尽可能压紧侧栏、会话列表、命令历史和传输文字；宽松保留更大间距。
- Retry Center 与传输面板优化：
  - Retry Center 底部操作按选择 / 导出 / 重试 / 删除分组，删除类按钮使用危险样式。
  - 宽松密度下传输面板标题和进度文字更清晰。
...
- 工作台主题色：
  - 设置 > 工作区可选择海洋蓝、淡紫色、薄荷绿、琥珀黄、玫瑰粉；整套工作台（表面、边框、选中态、主操作）都会跟随色调，而不只是按钮。
- UI 清晰度优化：
  - 提升面板对比度与选中态、按钮层级更清晰、分区标题增加图标，并改进空终端 / 空列表状态。
  - 修复会话面板齿轮图标对齐，并让列表与设置面板的滚动条重新可见。
...
- 特权远程文件写回：
  - 打开不可写远程文件（例如 `/etc` 下）时，可选择只读打开，或「暂存 + `sudo install`」写回。
  - 建议的 install 命令在成功后会用 `rm -f` 清理 staging 副本；`sudo -n` 自动写回成功后也会删除暂存文件。
- SFTP 路径跳转：
  - 在路径栏输入 `~` 或 `~/...` 会解析到远程 home，不再报 `No such file`。
- 更清晰的上传权限失败：
  - 拖拽/上传到不可写目录时直接报 `Permission denied`，不再静默改道 staging。
...
- 终端全屏编辑器闪烁修复：
  - `nano`、`crontab -e` 等 alternate-screen 工具不再触发编辑器聚焦布局的拉伸 / 缩回循环。
  - 修复闪烁后，编辑器聚焦的主题、排版、光标和全屏布局仍会正常生效。
  - 终端渲染迁移到 scoped xterm 包，并在可用时使用 WebGL 渲染；GPU 上下文不可用或丢失时会安全回退到 DOM 渲染。

### 验证

- `pnpm run typecheck` 已通过。
- `pnpm run build` 已通过。
- `pnpm run smoke:ui` 已通过（50/50）。
- WSL 环境中的 `nano` / `crontab -e` 手动验证已通过。
...
- 渲染层性能优化：
  - 启动更快：较重的弹窗（设置、Server Health 详情、新建会话、会话模板管理）改为按需加载，并对渲染产物分包以消除循环依赖 chunk，减小首屏体积。
  - 长列表更流畅：Retry Center 和命令历史管理器改为虚拟化渲染行，SFTP 与会话列表行做了 memo 化，避免无谓的重渲染。
  - 传输过程更顺滑：高频的 SFTP 传输进度更新现在按动画帧合并，状态变化与终止态仍会立即刷新。
...
- 工作台面板体验优化：
  - SFTP Details 视图中的修改时间现在会跟随应用界面语言显示。
  - SFTP Details 视图为文件名保留可用的最小宽度，并在侧栏较窄时支持横向滚动。
  - Transfers 面板在没有活跃传输任务时会保持更紧凑的高度。
  - 首次使用引导现在更清楚地提示导入主机、测试连接、打开终端 + SFTP 的路径。
  - Server Health 刷新时会保留已有数据，并用更轻量的刷新状态提示替代面板跳动。
...
- 重新整理了公开产品定位，强调 TermDock 是面向个人开发者和小团队的本地优先 SSH + SFTP 工作台。
- README 和 GitHub Release 文案移除了过期的“暂无应用内自动更新”描述，因为打包版本已经支持自动更新。
- 补充了更明确的下载指引，帮助用户更快判断该下载哪个 Windows 或 macOS 安装包。
- 把 README、产品记录、进度/任务摘要和 release 页面模板重新对齐到当前发布基线。
